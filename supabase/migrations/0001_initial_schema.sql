...
create type leave_type as enum ('annual', 'comp', 'special');
create type grant_source as enum ('auto', 'manual');
create type leave_duration as enum ('full', 'half_am', 'half_pm');
...
comment on table companies is '테넌트(회사) — SaaS 가입 단위. 모든 다른 테이블은 이 company_id로 격리됨.';
...
comment on table profiles is '직원 프로필. auth.users.id와 1:1로 매핑됨.';
...
create index idx_grants_company_employee on leave_grants(company_id, employee_id);
create index idx_grants_grant_date on leave_grants(grant_date);

comment on table leave_grants is '휴가 부여 기록 — 월별 자동 부여, 연간 일괄 부여, 대체휴무 수동 부여 등이 모두 이 테이블에 기록됨.';
...
create index idx_requests_company_employee on leave_requests(company_id, employee_id);
create index idx_requests_status on leave_requests(status);
create index idx_requests_use_date on leave_requests(use_date);

comment on table leave_requests is '휴가 신청 건. status=approved인 건만 잔여 차감에 반영됨.';
...
comment on view leave_balances is '실시간 잔여 계산. 테이블이 아닌 view여서 업데이트 책임 불필요.';
